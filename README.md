...
小组业务日报默认不自动推送。组长完成当天数据后，在组长工作台手动生成日报，再选择下载 Excel 或推送到 Telegram。只有服务器显式设置 `GROUP_DAILY_AUTO_PUSH_ENABLED=true` 时，内部定时任务才允许自动推送小组日报。

如果未来明确要恢复自动简报，生产服务器才可在 `crontab -e` 中加入下面一行（当前不应配置）：
