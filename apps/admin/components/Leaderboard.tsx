...
function RoleRankingTable({ title, note, rows, metric }: { title: string; note: string; rows: Row[]; metric: Metric }) {
  ...
  return <section className="card"><div className="card-head"><div><h2 className="card-title">{title}</h2><p className="card-note">{note}；红色预警表示低于同岗位平均值的一半。</p></div><span className="badge" data-tone="mute">同岗位平均 {metric === "netCents" ? money(Math.round(average)) : average.toFixed(1)}</span></div><div className="table-scroll" style={{ maxHeight: 430 }}><table className="grid-table"><thead><tr><th>排名</th><th>员工</th><th>所属小组</th><th>本期成绩</th><th>同岗位平均</th><th>达到平均</th><th>状态</th></tr></thead><tbody>{sorted.map((row, index) => {
    ...
    return <tr key={row.id}><td><strong className="tnum">{index + 1}</strong></td><td><strong>{row.name}</strong></td><td>{row.groupName ?? row.departmentName ?? "—"}</td><td className="tnum"><strong>{value(row, metric)}</strong></td><td className="tnum">{metric === "netCents" ? money(Math.round(average)) : average.toFixed(1)}</td><td className="tnum">{ratio === null ? "—" : `${(ratio * 100).toFixed(0)}%`}</td><td><span className="badge" data-tone={tone}>{status}</span></td></tr>;
