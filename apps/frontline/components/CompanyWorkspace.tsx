...
type Metrics = { added: number; collision: number; lowAmount: number; noWs: number; manualInvalid?: number; effective: number; replied: number; joined: number; leftNormal: number; leftAbnormal: number; inGroup: number; pushed: number; registered: number; ordered: number; initialDepositCents?: number; rechargeCents?: number; withdrawalCents: number; netCents: number };
type ReportGroup = { id: string; name: string; department: { id: string; name: string }; activePeople: number; totals: Metrics };
...
const EMPTY: Metrics = { added: 0, collision: 0, lowAmount: 0, noWs: 0, manualInvalid: 0, effective: 0, replied: 0, joined: 0, leftNormal: 0, leftAbnormal: 0, inGroup: 0, pushed: 0, registered: 0, ordered: 0, initialDepositCents: 0, rechargeCents: 0, withdrawalCents: 0, netCents: 0 };
...
function sum(rows: Array<{ totals: Metrics }>): Metrics { const result = { ...EMPTY }; for (const row of rows) for (const [key, value] of Object.entries(row.totals)) (result as unknown as Record<string, number>)[key] += Number(value) || 0; return result; }
...
export default function CompanyWorkspace({ user, onLogout }: { user: BackendUser; onLogout: () => void }) {
  ...
  const companyTotals = useMemo(() => sum(report?.groups ?? []), [report]);
  const departmentRows = useMemo(() => departments.map((department) => ({ name: department.name, sub: `${department.groups.length} 个小组`, people: (report?.groups ?? []).filter((group) => group.department.id === department.id).reduce((value, group) => value + group.activePeople, 0), totals: sum((report?.groups ?? []).filter((group) => group.department.id === department.id)) })), [departments, report]);
  ...
  const summaryRows = summaryMode === "department" ? departmentRows
    : summaryMode === "group" ? (report?.groups ?? []).map((group) => ({ name: group.name, sub: group.department.name, people: group.activePeople, totals: group.totals }))
      : summaryMode === "member" ? (report?.members ?? []).map((member) => ({ name: member.name, sub: `${groupDepartment.get(member.groupId) ?? "未知部门"} · ${member.groupName}`, totals: member.totals }))
        : summaryMode === "channel" ? (report?.channels ?? []).map((channel) => ({ name: channel.name, sub: `覆盖 ${channel.groupCount} 个小组`, totals: channel.totals }))
          : (report?.days ?? []).map((day) => ({ name: day.date, totals: sum(day.groups) }));
  ...
  return <WorkspaceShell mark="司" workspaceLabel="公司管理员" title={title} subtitle="只显示当前公司范围内的真实数据" userName={user.name} userLabel="公司管理员" onLogout={onLogout} scope={{ label: "公司管理范围", value: company?.name ?? user.companyName ?? "所属公司" }} navigation={<>
      ...
      <Nav active={view === "notifications"} icon="notifications" label={<>通知中心<NotificationBadge count={notificationUnread} /></>} onClick={() => setView("notifications")} />
        ...
        {!loading && view === "dashboard" ? <><section className={styles.kpis}>{[["部门", departments.length], ["小组", report?.groups.length ?? 0], ["有效数据", companyTotals.effective], ["进群", companyTotals.joined], ["开单", companyTotals.ordered], ["净业绩", money(companyTotals.netCents)]].map(([label, value]) => <article key={label}><span>{label}</span><strong>{value}</strong></article>)}</section><MetricTable title="部门经营概况" rows={departmentRows} /></> : null}
        {!loading && view === "summary" ? <><Tabs values={[["department", "按部门"], ["group", "按小组"], ["member", "按归属个人"], ["channel", "按渠道"], ["day", "按日期"]]} active={summaryMode} onChange={(value) => setSummaryMode(value as SummaryMode)} /><MetricTable title={`${summaryMode === "member" ? "个人归属数据汇总（每人一行）" : "公司数据汇总"} · ${report?.range.label ?? ""}`} rows={summaryRows} /></> : null}
...
function StructurePanel({ company, onDone }: { company: Company | null; onDone: (message: string) => void }) {
  ...
  async function createGroup(event: FormEvent<HTMLFormElement>) {
    ...
    try { await requestJson("/api/org/groups", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ departmentId, name: String(data.get("name") ?? "").trim() }) }); onDone("小组已创建。现在可以在下方为该小组开设组长账号。"); form.reset(); } catch (caught) { setGroupError(caught instanceof Error ? caught.message : "开组失败"); } finally { setGroupBusy(false); }
  }
