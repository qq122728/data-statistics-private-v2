...
export default function CompanyWorkspace({ user, onLogout }: { user: BackendUser; onLogout: () => void }) {
  ...
  const summaryRows = summaryMode === "department" ? departmentRows
    : summaryMode === "group" ? filteredGroups.map((group) => ({ name: group.name, sub: group.department.name, people: group.activePeople, totals: group.totals }))
  ...
  return <WorkspaceShell mark="司" workspaceLabel="公司管理员" title={title} subtitle="只显示当前公司范围内的真实数据" userName={user.name} userLabel="公司管理员" onLogout={onLogout} scope={{ label: "公司管理范围", value: company?.name ?? user.companyName ?? "所属公司" }} navigation={<>
      ...
      <Nav active={view === "notifications"} icon="notifications" label={<>通知中心<NotificationBadge count={notificationUnread} /></>} onClick={() => setView("notifications")} />
        ...
        {!loading && view === "summary" ? <><Tabs values={[["department", "按部门"], ["group", "按小组"], ["member", "按归属个人"], ["channel", "按渠道"], ["day", "按日期"]]} active={summaryMode} onChange={(value) => setSummaryMode(value as SummaryMode)} /><AdaptiveMetricTable groupType={groupTypeFilter} title={`${summaryMode === "member" ? "个人归属数据汇总（每人一行）" : "公司数据汇总"} · ${report?.range.label ?? ""}`} rows={summaryRows} /></> : null}
...
function AdaptiveMetricTable({ groupType, title, rows }: { groupType: "HACKER" | "LAWYER"; title: string; rows: MetricRow[] }) {
  return groupType === "LAWYER" ? <LawyerMetricTable title={title} rows={rows} /> : <MetricTable title={title} rows={rows} />;
}
function LawyerMetricTable({ title, rows }: { title: string; rows: MetricRow[] }) {
  ...
  return <section className="fresh-sheet-card"><div className="fresh-sheet-title"><div><h2>{title} · 律师组</h2><p>回复率和添加率都以接粉数为分母</p></div><div><span>共</span><strong>{rows.length} 行</strong></div></div><div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>名称</th><th>人数</th><th>接粉</th><th>回复</th><th>未回复</th><th>接粉小金额</th><th>真实案件</th><th>回复率</th><th>添加律师</th><th>添加专家</th><th>添加律师率</th><th>添加专家率</th><th>总推客服</th><th>总注册</th><th>总开单</th><th>加密货币充值</th><th>银行卡充值</th><th>出金</th></tr></thead><tbody>{rows.map((row) => <tr key={`${row.name}-${row.sub ?? ""}`}><td><strong>{row.name}</strong>{row.sub ? <small>{row.sub}</small> : null}</td>{cells(row)}</tr>)}<tr className={styles.total}><td><strong>合计</strong><small>当前 {rows.length} 行</small></td>{cells({ name: "合计", people, totals: total })}</tr></tbody></table></div></section>;
}
