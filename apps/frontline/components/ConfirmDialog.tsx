...
export function ConfirmDialog({
  ...
  const normalizedKindQuery = kindQuery.trim().toLocaleLowerCase();
  const visibleKindOptions = confirm.kindOptions?.filter((option) => (
    !normalizedKindQuery
    || option.label.toLocaleLowerCase().includes(normalizedKindQuery)
    || option.value.toLocaleLowerCase().includes(normalizedKindQuery)
  )) ?? [];
    ...
    >
      <div
        ...
        style={{
          ...
          maxHeight: "calc(100vh - 32px)", display: "flex", flexDirection: "column", overflow: "hidden",
        }}
      >
        ...
        {phase === "form" ? (
          <div style={{ padding: "0 20px 16px", display: "flex", flexDirection: "column", gap: 12, overflowY: "auto" }}>
            {confirm.target ? (
              <div
                style={{
            ...
            {confirm.kindOptions?.length ? (
              <div>
                ...
                {confirm.kindOptions.length > 12 ? (
                  <input
                    className="field"
                    value={kindQuery}
                    onChange={(event) => setKindQuery(event.target.value)}
                    placeholder={`搜索${confirm.kindLabel ?? "选项"}`}
                    aria-label={`搜索${confirm.kindLabel ?? "选项"}`}
                    style={{ width: "100%", marginBottom: 8 }}
                  />
                ) : null}
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", maxHeight: 240, overflowY: "auto", paddingRight: 4 }}>
                  {visibleKindOptions.map((o) => (
                    <button
                      key={o.value}
                      type="button"
                  ...
                  {!visibleKindOptions.length ? <span style={{ color: "var(--ink-3)", fontSize: 13 }}>没有找到对应选项</span> : null}
                </div>
              </div>
            ) : null}
