...
type Customer = {
  ...
  groupQueueNumber: number | null; expertQueueNumber: number | null;
...
type ColumnKey = "dates" | "customer" | "attribution" | "maintenance" | "days" | "groupProgress" | "leave" | "expertOwner" | "expertProgress" | "registration" | "initial" | "recharge" | "withdrawal" | "net" | "updated";

const mandatoryColumns = new Set<ColumnKey>(["dates", "customer", "attribution", "expertOwner", "updated"]);
const columnLabels: Record<ColumnKey, string> = {
  dates: "客户日期", customer: "客户信息", attribution: "客户归属", maintenance: "群维护", days: "群内天数", groupProgress: "炒群情况", leave: "退群信息", expertOwner: "专家负责人",
  expertProgress: "专家情况", registration: "注册信息", initial: "首充", recharge: "续充", withdrawal: "出金", net: "净业绩", updated: "最后修改",
};
const columnsByView: Record<ViewMode, ColumnKey[]> = {
  group: ["dates", "customer", "attribution", "maintenance", "days", "groupProgress", "leave", "expertOwner", "updated"],
  expert: ["dates", "customer", "attribution", "leave", "expertOwner", "expertProgress", "registration", "initial", "recharge", "withdrawal", "net", "updated"],
};
const compactHiddenByView: Record<ViewMode, ColumnKey[]> = { group: ["leave"], expert: ["leave", "recharge", "withdrawal"] };

const money = (cents = 0) => new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(cents / 100);
const queueNumber = (prefix: "G" | "E", value: number | null) => value ? `${prefix}-${String(value).padStart(3, "0")}` : "—";
...
export function DepartmentCustomerProgress({ groups, member }: { groups?: DepartmentCustomerGroup[]; member?: BackendUser }) {
  ...
  const [hiddenColumns, setHiddenColumns] = useState<ColumnKey[]>(compactHiddenByView.group);
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true; };
  }, [groupId, page, query, reloadKey, viewMode]);
  ...
  useEffect(() => {
    try {
      const saved = window.localStorage.getItem(`customer-progress-hidden-${viewMode}`);
      setHiddenColumns(saved ? JSON.parse(saved) as ColumnKey[] : compactHiddenByView[viewMode]);
    } catch { setHiddenColumns(compactHiddenByView[viewMode]); }
  }, [viewMode]);

  const customers = useMemo(() => (payload?.customers ?? []).filter((customer) => {
    const haystack = `${customer.phone} ${queueNumber("G", customer.groupQueueNumber)} ${queueNumber("E", customer.expertQueueNumber)} ${customer.customerName ?? ""} ${customer.customerPlatform ?? ""} ${customer.attributionOwner?.name ?? customer.owner?.name ?? ""} ${customer.batch.channel.name} ${customer.groupOperatorOwner?.name ?? ""} ${customer.expertOwner?.name ?? ""} ${latestGroupText(customer)} ${latestExpertText(customer)}`.toLowerCase();
  ...
  const showColumn = (key: ColumnKey) => !compactView || mandatoryColumns.has(key) || !hiddenColumns.includes(key);
  ...
  function toggleColumn(key: ColumnKey) {
    ...
    setHiddenColumns((current) => {
      ...
      try { window.localStorage.setItem(`customer-progress-hidden-${viewMode}`, JSON.stringify(next)); } catch { /* 仍保留本次页面设置 */ }
      return next;
    });
  }
  ...
  return <div className={styles.sheetWorkspace}>
    <section className={styles.toolbar}>
      <label className={styles.search}><MagnifyingGlass size={14} /><input aria-label="搜索客户" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="搜索号码、G/E 编号、组员或进度" /></label>
      ...
      <div className={styles.densitySwitch} aria-label="表格显示密度"><button type="button" data-active={compactView} onClick={() => setCompactView(true)}>精简视图</button><button type="button" data-active={!compactView} onClick={() => setCompactView(false)}>完整视图</button></div>
      <details className={styles.columnPicker}><summary>显示字段 <b>{tableColumns}/{columnsByView[viewMode].length}</b></summary><div>{columnsByView[viewMode].map((key) => <label key={key}><input type="checkbox" checked={showColumn(key)} disabled={mandatoryColumns.has(key)} onChange={() => toggleColumn(key)} /><span>{columnLabels[key]}</span>{mandatoryColumns.has(key) ? <small>固定</small> : null}</label>)}</div></details>
          ...
          <tbody>
            ...
            {loading ? <tr><td colSpan={tableColumns} className={styles.empty}>正在读取组内共享数据…</td></tr> : customers.map((customer) => {
              ...
              return <tr key={customer.id}>
                {showColumn("dates") ? <td><div className={styles.stackedCell}><div className={styles.queueLine}><span>群号</span><b className={styles.queueBadge} data-kind="group">{queueNumber("G", customer.groupQueueNumber)}</b></div>{viewMode === "expert" ? <div className={styles.queueLine}><span>专家号</span><b className={styles.queueBadge} data-kind="expert">{queueNumber("E", customer.expertQueueNumber)}</b></div> : null}<label><span>接粉</span>{canEdit ? <input key={customer.batch.sourceDate} className={styles.dateInput} type="date" defaultValue={customer.batch.sourceDate} max={customer.joinedOn ?? localToday()} disabled={Boolean(savingCell)} onChange={(event) => event.target.value && event.target.value !== customer.batch.sourceDate && void patchCell(customer, { action: "setSourceDate", occurredOn: event.target.value }, "sourceDate", "接粉日期已保存")} /> : <b>{customer.batch.sourceDate}</b>}</label><label><span>进群</span>{canEdit ? <input key={customer.joinedOn ?? "empty"} className={styles.dateInput} type="date" defaultValue={customer.joinedOn ?? ""} min={customer.batch.sourceDate} max={customer.leftOn ?? localToday()} disabled={Boolean(savingCell)} onChange={(event) => event.target.value && event.target.value !== customer.joinedOn && void patchCell(customer, { action: "setJoinedOn", occurredOn: event.target.value }, "joinedOn", "进群日期已保存")} /> : <b>{customer.joinedOn ?? "—"}</b>}</label></div></td> : null}
                {showColumn("customer") ? <td className={styles.phone}><div className={styles.customerInfo}><strong>{customer.phone}</strong>{canEdit ? <><input key={customer.customerName ?? "empty-name"} className={styles.customerNameInput} defaultValue={customer.customerName ?? ""} maxLength={80} placeholder="客户姓名" disabled={Boolean(savingCell)} onBlur={(event) => { const customerName = event.target.value.trim(); if (customerName !== (customer.customerName ?? "")) void patchCell(customer, { action: "setCustomerName", customerName }, "customerName", "客户姓名已保存"); }} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} /><input key={customer.customerPlatform ?? "empty-platform"} className={styles.customerNameInput} defaultValue={customer.customerPlatform ?? ""} maxLength={100} placeholder="平台名称" disabled={Boolean(savingCell)} onBlur={(event) => { const customerPlatform = event.target.value.trim(); if (customerPlatform !== (customer.customerPlatform ?? "")) void patchCell(customer, { action: "setCustomerPlatform", customerPlatform }, "customerPlatform", "客户平台已保存"); }} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} /><input key={customer.lossAmountCents ?? "empty-amount"} className={styles.customerNameInput} type="number" min="0" step="0.01" defaultValue={customer.lossAmountCents === null ? "" : customer.lossAmountCents / 100} placeholder="被骗金额" disabled={Boolean(savingCell)} onBlur={(event) => { const value = event.target.value.trim(); const amountCents = value ? Math.round(Number(value) * 100) : null; if ((amountCents === null || Number.isFinite(amountCents)) && amountCents !== customer.lossAmountCents) void patchCell(customer, { action: "setLossAmount", amountCents }, "lossAmount", "被骗金额已保存"); }} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} /></> : <><small>{customer.customerName?.trim() || "未填写姓名"}</small><small>{customer.customerPlatform?.trim() || "未填写平台"}</small><small>{customer.lossAmountCents === null ? "未填写被骗金额" : `被骗金额 ${money(customer.lossAmountCents)}`}</small></>}</div></td> : null}
                {showColumn("attribution") ? <td><div className={styles.stackedCell}><label><span>组员</span>{canEdit ? <select className={styles.cellSelect} value={attributedOwner?.id ?? ""} disabled={Boolean(savingCell)} onChange={(event) => void patchCell(customer, { action: "setOwner", userId: event.target.value }, "owner", "接粉及业绩归属已保存")}>{payload?.memberOptions.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select> : <b>{attributedOwner?.name ?? "未分配"}</b>}</label><label><span>渠道</span>{canEdit ? <select className={styles.cellSelect} value={payload?.channelOptions.find((item) => item.name === customer.batch.channel.name)?.id ?? ""} disabled={Boolean(savingCell)} onChange={(event) => void patchCell(customer, { action: "setChannel", channelId: event.target.value }, "channel", "来源渠道已保存")}>{payload?.channelOptions.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select> : <b>{customer.batch.channel.name}</b>}</label></div></td> : null}
                {viewMode === "group" ? <>
                {showColumn("maintenance") ? <td><div className={styles.stackedCell}><label><span>负责人</span>{canEdit ? <select className={styles.cellSelect} value={customer.groupOperatorOwner?.id ?? ""} disabled={Boolean(savingCell)} onChange={(event) => void patchCell(customer, { action: "assignGroupOperator", userId: event.target.value }, "operator", "炒群负责人已保存")}><option value="" disabled>点击选择</option>{payload?.memberOptions.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select> : <b>{customer.groupOperatorOwner?.name ?? "待分配"}</b>}</label><label><span>设备号</span>{canEdit ? <input key={customer.device?.code ?? "empty"} className={styles.deviceInput} defaultValue={customer.device?.code ?? ""} maxLength={100} placeholder="手动填写" disabled={Boolean(savingCell)} onBlur={(event) => { const code = event.target.value.trim(); if (code !== (customer.device?.code ?? "")) void patchCell(customer, { action: "setDeviceCode", code }, "device", "设备号已保存"); }} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} /> : <b>{customer.device?.code ?? "—"}</b>}</label></div></td> : null}
                {showColumn("days") ? <td className={styles.dayCell}>{daysInGroup(customer.joinedOn, customer.leftOn)}</td> : null}
                {showColumn("groupProgress") ? <td className={styles.progressCell}><EditableCell label="炒群情况" value={latestGroupText(customer)} editable={canEdit} saving={savingCell === `${customer.id}:group`} onSave={(note) => saveSituation(customer, "group", note)} /></td> : null}
                </> : null}
                {showColumn("leave") ? <td><div className={styles.stackedCell}><label><span>类型</span>{canEdit ? <select className={styles.cellSelect} value={customer.groupStatus === "LEFT" ? (customer.leftWithOrder ? "NORMAL" : "ABNORMAL") : ""} disabled={Boolean(savingCell)} onChange={(event) => { const leaveType = event.target.value; void patchCell(customer, leaveType ? { action: "setLeave", leaveType, occurredOn: customer.leftOn ?? localToday() } : { action: "setLeave", leaveType: "NONE" }, "leave", leaveType ? "退群类型已纠正" : "退群记录已撤销"); }}><option value="">{customer.groupStatus === "LEFT" ? "撤销退群" : "—"}</option><option value="NORMAL">正常退群</option><option value="ABNORMAL">异常退群</option></select> : <b>{customer.groupStatus === "LEFT" ? (customer.leftWithOrder ? "正常退群" : "异常退群") : "—"}</b>}</label><label><span>日期</span>{canEdit && customer.groupStatus === "LEFT" ? <input key={customer.leftOn ?? "empty-left"} className={styles.dateInput} type="date" defaultValue={customer.leftOn ?? ""} min={customer.joinedOn ?? undefined} max={localToday()} disabled={Boolean(savingCell)} onChange={(event) => event.target.value && event.target.value !== customer.leftOn && void patchCell(customer, { action: "setLeave", leaveType: customer.leftWithOrder ? "NORMAL" : "ABNORMAL", occurredOn: event.target.value }, "leave", "退群日期已纠正")} /> : <b>{customer.leftOn ?? "—"}</b>}</label></div></td> : null}
                {showColumn("expertOwner") ? <td>{canEdit ? <select className={styles.cellSelect} value={customer.expertOwner?.id ?? ""} disabled={Boolean(savingCell)} onChange={(event) => void patchCell(customer, { action: "assignExpert", userId: event.target.value }, "expert", "专家负责人已保存")}><option value="" disabled>点击选择</option>{payload?.expertOptions.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select> : customer.expertOwner?.name ?? "未分配"}</td> : null}
                {viewMode === "expert" ? <>
                {showColumn("expertProgress") ? <td className={styles.progressCell}><EditableCell label="专家情况" value={latestExpertText(customer)} editable={canEditExpert} saving={savingCell === `${customer.id}:expert`} onSave={(note) => saveSituation(customer, "expert", note)} /></td> : null}
                {showColumn("registration") ? <td><div className={styles.stackedCell}><div><span>状态</span><span className={styles.registrationStatus} data-registered={Boolean(customer.registeredOn)}>{customer.registeredOn ? "已注册" : "未注册"}</span></div><label><span>日期</span>{canEditExpert ? <input key={customer.registeredOn ?? "empty"} className={styles.dateInput} type="date" defaultValue={customer.registeredOn ?? ""} min={customer.joinedOn ?? undefined} disabled={Boolean(savingCell)} onChange={(event) => event.target.value && void patchCell(customer, { action: "setRegistration", occurredOn: event.target.value }, "registration", "注册日期已保存")} /> : <b>{customer.registeredOn ?? "—"}</b>}</label></div></td> : null}
                {showColumn("initial") ? <td className={styles.money}>{customer.order ? <button className={styles.financeCell} onClick={() => canEditExpert && openFinance("INITIAL", customer)} disabled={!canEditExpert}>{money(customer.order.initialDepositCents)}</button> : <button className={styles.financeAdd} disabled={!canEditExpert || !customer.registeredOn} title={!canEditExpert ? "需要专家权限" : !customer.registeredOn ? "请先填写注册日期" : "登记首充"} onClick={() => openFinance("INITIAL", customer)}>+ 首充</button>}</td> : null}
                {showColumn("recharge") ? <td className={styles.money}><button className={styles.financeCell} disabled={!canEditExpert || !customer.order} onClick={() => openFinance("RECHARGE", customer)}>{money(customer.order?.rechargeCents)}{rechargeCount ? <small>{rechargeCount}笔</small> : null}</button></td> : null}
                {showColumn("withdrawal") ? <td><button className={styles.financeCell} disabled={!canEditExpert || !customer.order} onClick={() => openFinance("WITHDRAWAL", customer)}>{money(customer.order?.withdrawalCents)}{withdrawalCount ? <small>{withdrawalCount}笔</small> : null}</button></td> : null}
                {showColumn("net") ? <td className={styles.net}>{money(net)}</td> : null}
                </> : null}
                {showColumn("updated") ? <td className={styles.updated}>{customer.activities[0]?.actor?.name ?? "系统"}<small>{customer.activities[0]?.occurredOn ?? "—"}</small></td> : null}
              </tr>;
            })}
