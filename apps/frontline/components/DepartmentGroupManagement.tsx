...
export default function DepartmentGroupManagement() {
  ...
  async function submitCreate(event: FormEvent<HTMLFormElement>) {
    ...
    const groupType = form.get("groupType") === "LAWYER" ? "LAWYER" : "HACKER";
    setBusy(true); setError("");
    try {
      await requestJson("/api/org/groups", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ departmentId: department.id, name, groupType }) });
  ...
  return <div className="department-management">
    ...
    {createOpen ? <div className="department-modal-backdrop"><form className="department-modal" onSubmit={submitCreate}><header><div><h2>第一步：开设新组</h2><p>小组类型决定组员使用哪一套每日统计指标</p></div><button type="button" onClick={() => setCreateOpen(false)}>×</button></header><label><span>小组名称</span><input name="name" required autoFocus /></label><label><span>小组类型</span><select name="groupType" defaultValue="HACKER"><option value="HACKER">黑客组（现有统计表）</option><option value="LAWYER">律师组（律师业务统计表）</option></select></label>{error ? <p className="department-error-text">{error}</p> : null}<footer><button type="button" onClick={() => setCreateOpen(false)}>取消</button><button className="fresh-primary" disabled={busy}>{busy ? "保存中…" : "确认创建小组"}</button></footer></form></div> : null}
