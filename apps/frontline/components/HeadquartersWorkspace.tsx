...
function MetricTable({ title, rows }: { title: string; rows: TableRow[] }) {
  ...
  return <section className={styles.card}><div className={styles.cardHead}><div><h2>{title}</h2><p>底部合计只汇总当前公司、部门、小组筛选范围；转化率按统一业务口径重新计算</p></div><strong>{rows.length} 行</strong></div><div className={styles.metricTable}><table><thead><tr><th>名称</th><th>人数</th><th>添加</th><th>撞粉</th><th>低金额</th><th>无 WS</th><th>人工无效</th><th>有效</th><th>回复</th><th>回复率</th><th>进群</th><th>进群率</th><th>正常退群</th><th>异常退群</th><th>异常退群率</th><th>在群</th><th>推专家</th><th>注册</th><th>注册率</th><th>开单</th><th>开单率</th><th>首充</th><th>续充</th><th>出金</th><th>净业绩</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td><strong>{row.name}</strong>{row.sub ? <small>{row.sub}</small> : null}</td>{cells(row)}</tr>)}<tr className={styles.total}><td><strong>合计</strong><small>当前显示 {rows.length} 行</small></td>{cells({ totals, people: people ?? undefined })}</tr></tbody></table></div></section>;
}
...
function ManagerAccounts({ companies, notify }: { companies: CompanyNode[]; notify: (value: string) => void }) {
  ...
  const [name, setName] = useState(""); const [username, setUsername] = useState(""); const [password, setPassword] = useState(temporaryPassword); const [error, setError] = useState(""); const [busy, setBusy] = useState(false);
  const load = useCallback(async () => {
