...
export function HeadquartersWorkspace({ user, onLogout }: { user: BackendUser; onLogout: () => void }) {
  ...
  return <WorkspaceShell mark="总" workspaceLabel="总公司管理员" title={title[view]} subtitle="总公司权限 · 所有写操作仍由后端再次核验范围" userName={user.name} userLabel="总公司管理员" onLogout={onLogout} assistant={<AiSmartAssistant open={aiOpen} onOpenChange={setAiOpen} contextLabel={`当前页面 · ${title[view]}`} user={user} />} scope={{ label: "管理范围", value: `全部公司 · ${companies.length} 家` }} navigation={<>
        ...
        {(view === "dashboard" || view === "summary") ? <div className={view === "dashboard" ? styles.compactDate : styles.standardDate}><SmartDateRangeToolbar range={range} from={from} to={to} currentLabel={report?.range.label} loading={loading} title="总公司统计日期" onRange={setRange} onFrom={setFrom} onTo={setTo} onRefresh={() => void load()} /></div> : null}
        {(view === "dashboard" || view === "summary") ? <div className={`${styles.tabs} ${view === "dashboard" ? styles.compactTabs : ""}`}><button data-active={groupTypeFilter === "HACKER"} onClick={() => { setGroupTypeFilter("HACKER"); setGroupId(""); setDetailGroupId(""); }}>黑客组数据</button><button data-active={groupTypeFilter === "LAWYER"} onClick={() => { setGroupTypeFilter("LAWYER"); setGroupId(""); setDetailGroupId(""); }}>律师组数据</button></div> : null}
        {loading ? <section className={styles.empty}>正在读取全部公司数据…</section> : null}
        {!loading && view === "dashboard" ? <Dashboard groupType={groupTypeFilter} rows={dashboardRows} groups={(report?.groups ?? []).filter((group) => group.groupType === groupTypeFilter)} /> : null}
