...
type Context = { unifiedEntries: UnifiedEntry[] };
...
export function MemberDailyRecords({ mode: _mode }: { mode: "history" | "finance" }) {
  ...
  return <div className="member-history">
    ...
    <section className="card member-history__card">
      <div className="table-scroll"><table className="grid-table member-history__table">
        ...
        <tbody>
          {loading && !rows.length ? <tr><td colSpan={19} className="member-history__empty">正在读取真实历史数据…</td></tr> : null}
          {!loading && !rows.length ? <tr><td colSpan={19} className="member-history__empty">所选日期没有填写过数据。</td></tr> : null}
          {rows.map((row) => {
            const effective = Math.max(0, row.dispatchCount - row.duplicateCount - row.lowAmountCount - row.noWsCount - row.manualInvalidCount);
            const current = Math.max(0, row.joinCount - row.normalLeaveCount - row.abnormalLeaveCount);
            const first = row.cryptoInitialDepositCents + row.bankInitialDepositCents;
            const recharge = row.cryptoRechargeCents + row.bankRechargeCents;
            return <tr key={row.key}>
              <td><strong>{row.businessDate}</strong></td><td>{row.channelName}</td><td>{row.dispatchCount}</td><td><strong>{effective}</strong></td>
              <td>{row.replyCount}</td><td>{percent(row.replyCount, effective)}</td><td>{row.joinCount}</td><td>{percent(row.joinCount, effective)}</td>
              <td>{percent(row.abnormalLeaveCount, Math.max(0, row.joinCount - row.normalLeaveCount))}</td><td>{current}</td><td>{row.expertIntroCount}</td>
              <td>{row.registrationCount}</td><td>{percent(row.registrationCount, row.expertIntroCount)}</td><td>{row.orderCount}</td><td>{percent(row.orderCount, row.registrationCount)}</td>
              <td>{money(first)}</td><td>{money(recharge)}</td><td>{money(row.withdrawalCents)}</td><td><strong>{money(first + recharge - row.withdrawalCents)}</strong></td>
            </tr>;
          })}
        </tbody>
      </table></div>
