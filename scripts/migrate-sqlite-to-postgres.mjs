...
export const copyPlan = [
  ...
  ["teamGroup", "TeamGroup"],
  ["customerStageSequence", "CustomerStageSequence"],
  ["customerDailyStageSequence", "CustomerDailyStageSequence"],
  ["user", "User"],
  ["userManagedDepartment", "UserManagedDepartment"],
  ["userRoleAssignment", "UserRoleAssignment"],
  ["userGroupMembership", "UserGroupMembership"],
  ["userPosition", "UserPosition"],
  ["groupLeadChangePlan", "GroupLeadChangePlan"],
  ["channel", "Channel"],
...
function differingFields(sourceRows, targetRows) {
  const targetById = new Map(
    targetRows.map((row) => [row.id, normalizeValue(row)]),
  );
  for (const sourceRow of sourceRows.map(normalizeValue)) {
    ...
    const fields = [
      ...
      (field) =>
        JSON.stringify(sourceRow[field]) !== JSON.stringify(targetRow[field]),
    );
...
async function copyModel(target, model, label, rows) {
  ...
  for (const batch of chunks(data)) {
    ...
    if (count !== batch.length) {
      throw new Error(
        `${label} 复制行数不一致：读取 ${batch.length} 行，但仅写入 ${count} 行`,
      );
    }
  }
}
...
async function verifyCopiedModels(target, sourceRowsByModel) {
  for (const [model, label] of copyPlan) {
    ...
    if (targetRows.length !== sourceRows.length) {
      throw new Error(
        `${label} 校验失败：SQLite 有 ${sourceRows.length} 行，PostgreSQL 有 ${targetRows.length} 行`,
      );
    }
    if (
      JSON.stringify(normalizedRows(targetRows)) !==
      JSON.stringify(normalizedRows(sourceRows))
    ) {
      const difference = differingFields(sourceRows, targetRows);
      throw new Error(
        `${label} 校验失败：记录 ${difference.id} 的字段 ${difference.fields.join(", ")} 与 SQLite 源数据不一致`,
      );
    }
...
export async function migrateSqliteToPostgres({ source, target }) {
  ...
  await target.$transaction(
    async (transaction) => {
      ...
      if (sessionCount !== 0)
        throw new Error(
          `Session 清理失败：目标库仍有 ${sessionCount} 条旧登录会话`,
        );
    },
...
async function main() {
  const postgresUrl = assertLocalPostgresTestDatabaseUrl(
