...
export async function GET(request: Request) {
  ...
  if (!["ADMIN", "COMPANY_MANAGER", "FINANCE", "LEAD"].includes(user.role)) {
    return authorizationDenied(user, "只有组长、公司管理员、总公司管理员和财务可以导出每日数据报表");
  }
  ...
  const payload = await loadMemberPerformanceExport({ from: date, to: date, groupIds });
  const workbook = await buildMemberPerformanceWorkbook({ from: date, to: date, reportType: "daily", ...payload });
  const bytes = await workbook.xlsx.writeBuffer();
  const fileName = `小组每日数据报表-${date}.xlsx`;
