...
const updateSchema = z.discriminatedUnion("action", [
  ...
  z.object({
    action: z.literal("setDeviceCode"),
    code: z.string().trim().max(100, "设备号不能超过 100 个字"),
  }),
  z.object({
  ...
  z.object({
    action: z.literal("setCustomerPlatform"),
    customerPlatform: z.string().trim().max(100, "平台名称不能超过 100 个字"),
  }),
  z.object({
    action: z.literal("setLossAmount"),
    amountCents: z
      .number()
      .int()
      .min(0, "被骗金额不能小于 0")
  ...
  z.object({
    action: z.literal("setRegistration"),
    occurredOn: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  }),
  z.object({
    action: z.literal("setLeave"),
    leaveType: z.enum(["NORMAL", "ABNORMAL", "NONE"]),
    occurredOn: z
      .string()
      .regex(/^\d{4}-\d{2}-\d{2}$/)
      .optional(),
...
export async function PATCH(
  ...
  if (
    !sessionUser.active ||
    !sessionUser.groupId ||
    !isFrontlineGroupMember(sessionUser)
  )
  ...
  try {
    ...
    const result = await db.$transaction(async (transaction) => {
      ...
      const lead = await transaction.leadCustomer.findUnique({
        ...
        include: {
          batch: {
            select: { groupId: true, channelId: true, sourceDate: true },
          },
      ...
      if (input.action === "assignGroupOperator") {
        ...
        if (!input.code) {
          ...
          const device = await transaction.device.upsert({
            where: {
              groupId_code: { groupId: actor.groupId, code: input.code },
            },
            update: {},
            create: {
              groupId: actor.groupId,
        ...
        if (lead.leftOn && occurredOn > lead.leftOn)
          return { status: 400 as const, error: "推专家日期不能晚于退群日期" };
        if (lead.registeredOn && occurredOn > lead.registeredOn)
          return { status: 400 as const, error: "推专家日期不能晚于注册日期" };
        if (
          lead.customerOrder &&
          !lead.customerOrder.voidedAt &&
          occurredOn > lead.customerOrder.openedOn
        )
          return { status: 400 as const, error: "推专家日期不能晚于开单日期" };
        trackedExpertOwnerId = target.id;
        if (!lead.groupQueueNumber) {
          update.groupQueueNumber = await allocateCustomerStageNumber(
            transaction,
            actor.groupId,
            "GROUP",
            lead.joinedOn ?? today,
        ...
        if (
          ...
          lead.expertIntroducedOn !== occurredOn
        ) {
          update.expertQueueNumber = await allocateCustomerStageNumber(
            transaction,
            actor.groupId,
            "EXPERT",
            occurredOn,
        ...
        activity = {
          ...
          note: input.customerName
            ? `客户姓名调整为 ${input.customerName}`
            : "客户姓名已清空",
          occurredOn: today,
        };
      } else if (input.action === "setPhone") {
        let phone: string;
        try {
        ...
        if (duplicate)
          return { status: 409 as const, error: "该客户号码已存在，不能重复" };
        update.phone = phone;
        activity = {
          kind: "PLAN_UPDATED",
          note: `客户号码由 ${lead.phone} 纠正为 ${phone}`,
          occurredOn: today,
        };
        ...
        activity = {
          kind: "PLAN_UPDATED",
          note: input.customerPlatform
            ? `客户平台调整为 ${input.customerPlatform}`
            : "客户平台已清空",
          occurredOn: today,
        };
        ...
        if (lead.leftOn && input.occurredOn > lead.leftOn)
          return { status: 400 as const, error: "进群日期不能晚于退群日期" };
        if (
          lead.expertIntroducedOn &&
          input.occurredOn > lead.expertIntroducedOn
        )
          return { status: 400 as const, error: "进群日期不能晚于推专家日期" };
        if (lead.registeredOn && input.occurredOn > lead.registeredOn)
          return { status: 400 as const, error: "进群日期不能晚于注册日期" };
        if (lead.joinedOn !== input.occurredOn || !lead.groupQueueNumber)
          update.groupQueueNumber = await allocateCustomerStageNumber(
            transaction,
            actor.groupId,
            "GROUP",
        ...
        activity = {
          kind: "PLAN_UPDATED",
          note: `进群日期调整为 ${input.occurredOn}`,
          occurredOn: today,
        };
      } else if (input.action === "setRegistration") {
        if (!lead.expertOwnerId)
          return { status: 400 as const, error: "请先选择专家负责人" };
        const dateError = entryDateError(input.occurredOn, today, "注册日期");
        if (dateError) return { status: 400 as const, error: dateError };
        ...
        if (
          lead.customerOrder &&
          !lead.customerOrder.voidedAt &&
          input.occurredOn > lead.customerOrder.openedOn
        )
          return { status: 400 as const, error: "注册日期不能晚于开单日期" };
        if (lead.registeredOn === input.occurredOn)
          return { status: 200 as const };
        update.registeredOn = input.occurredOn;
        update.registrationQueueNumber = await allocateCustomerStageNumber(
          transaction,
          actor.groupId,
          "REGISTRATION",
          input.occurredOn,
        );
        update.registrationQueueGroupId = actor.groupId;
        ...
        if (input.leaveType === "NONE") {
          if (!lead.leftOn)
            return { status: 400 as const, error: "该客户当前没有退群记录" };
          update.groupStatus = "JOINED";
          update.leftOn = null;
          update.leftWithOrder = null;
          update.leftNote = null;
          update.leftAutomatically = false;
          update.leaveQueueNumber = null;
          update.leaveQueueGroupId = null;
          ...
          if (!input.occurredOn)
            return { status: 400 as const, error: "请填写退群日期" };
          const dateError = entryDateError(input.occurredOn, today, "退群日期");
          if (dateError) return { status: 400 as const, error: dateError };
          if (lead.joinedOn && input.occurredOn < lead.joinedOn)
            return { status: 400 as const, error: "退群日期不能早于进群日期" };
          update.groupStatus = "LEFT";
          update.leftOn = input.occurredOn;
          update.leftWithOrder = input.leaveType === "NORMAL";
          update.leftAutomatically = false;
          if (lead.leftOn !== input.occurredOn || !lead.leaveQueueNumber)
            update.leaveQueueNumber = await allocateCustomerStageNumber(
              transaction,
              actor.groupId,
              "LEAVE",
              input.occurredOn,
            );
          update.leaveQueueGroupId = actor.groupId;
          if (lead.isHistoricalRecord) update.historicalLeaveCounted = true;
          activity = {
            kind: "LEFT_GROUP",
            note: lead.leftOn
              ? `纠错：退群调整为${input.leaveType === "NORMAL" ? "正常" : "异常"}退群，日期 ${input.occurredOn}`
              : input.leaveType === "NORMAL"
                ? "标记正常退群"
                : "标记异常退群",
            occurredOn: input.occurredOn,
          };
      ...
      if (activity)
        await transaction.leadActivity.create({
          data: { leadId: lead.id, actorId: actor.id, ...activity },
        });
      const trackedLead = {
        ...lead,
      ...
      if (
        ...
        if (lead.joinedOn)
          await syncCustomerGroupEvent(transaction, trackedLead, {
            businessDate: lead.joinedOn,
            kind: "JOIN",
            delta: -1,
          });
        await syncCustomerGroupEvent(transaction, trackedLead, {
          businessDate: input.occurredOn,
          kind: "JOIN",
        });
      } else if (input.action === "assignExpert") {
        const occurredOn =
        ...
        if (!lead.expertIntroducedOn) {
          ...
          await syncCustomerExpertEvent(transaction, trackedLead, {
            businessDate: occurredOn,
            kind: "RECEIVED",
          });
        } else if (lead.expertIntroducedOn !== occurredOn) {
          await moveCustomerExpertIntroductionDate(
            transaction,
            {
              ...lead,
              batch: {
            ...
            trackedLead,
            { from: lead.expertIntroducedOn, to: occurredOn },
          );
        }
      } else if (input.action === "setRegistration") {
        if (lead.registeredOn)
          await syncCustomerRegistrationEvent(
            transaction,
            trackedLead,
            lead.registeredOn,
            -1,
        ...
        if (input.leaveType !== "NONE" && input.occurredOn)
          await syncCustomerGroupEvent(transaction, trackedLead, {
            businessDate: input.occurredOn,
            kind:
              input.leaveType === "NORMAL" ? "NORMAL_LEAVE" : "ABNORMAL_LEAVE",
          });
      }
      await recordAudit(transaction, {
        actorId: actor.id,
        action: `SHARED_CUSTOMER_${input.action}`,
        ...
        summary: {
          ...
          before: {
            ...
            attributionOwnerId: lead.attributionOwnerId,
    ...
    if ("error" in result) {
      if (result.status === 403)
        return authorizationDenied(sessionUser, result.error);
