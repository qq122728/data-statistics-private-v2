    ...
    <aside role="dialog" aria-modal="true" aria-label={member ? "编辑成员" : "添加成员"} className="absolute inset-y-0 right-0 flex w-full max-w-sm flex-col border-l border-slate-200 bg-white shadow-2xl">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        <form id="member-form" onSubmit={submit} className="space-y-5">
          ...
          {role === "RESOURCE_MANAGER" ? <fieldset className="rounded-lg border border-cyan-200 bg-cyan-50 p-3"><legend className="px-1 text-sm font-semibold text-cyan-950">资源账号类型（必须且只能选择一个）</legend><p className="mb-2 text-xs leading-5 text-cyan-800">投流资源与短信资源严格分开。选定类型后，账号只能查看现在及以后该类型的授权渠道。</p><div className="grid gap-2 sm:grid-cols-2">{(["SMS", "ADS", "REBATE"] as ResourceChannelType[]).filter((channelType) => resourceChannels.some((channel) => channel.channelType === channelType)).map((channelType) => { const count = new Set(resourceChannels.filter((channel) => channel.channelType === channelType).map((channel) => channel.id)).size; return <label key={channelType} className="flex items-center gap-2 rounded-md border border-cyan-100 bg-white px-3 py-3 text-sm"><input type="radio" name="resourceChannelType" checked={resourceChannelTypes[0] === channelType} onChange={() => setResourceChannelTypes([channelType])} /><span className="font-semibold text-slate-800">{resourceChannelTypeNames[channelType]}</span><small className="ml-auto text-slate-400">{count} 个渠道</small></label>; })}</div>{!resourceChannels.length ? <p className="text-sm text-amber-700">请先在渠道管理中创建渠道。</p> : null}</fieldset> : null}
