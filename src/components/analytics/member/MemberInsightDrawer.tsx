...
export function MemberInsightDrawer({ memberId, memberName, role, query, onClose }: {
  ...
  const drawer = <Drawer title={evidence ? `${evidence.member.name}·组员详情` : `${memberName}·组员详情`} open={Boolean(memberId)} onClose={closeDrawer} className="member-insight-drawer max-w-2xl max-sm:max-w-none">
    ...
    {evidence && funnelRates ? <div className="space-y-6 p-5">
      ...
      <section><h3 className="font-semibold text-slate-950">渠道构成</h3><div className="mt-3 space-y-2">{evidence.channels.length ? evidence.channels.map((channel) => <article key={channel.channel.id} className="rounded-lg border border-slate-200 p-3 text-sm"><strong>{channel.channel.name}</strong><p className="mt-1 text-xs text-slate-500">有效数据 {channel.totals.effectiveFans} · 开单 {channel.totals.orders}</p></article>) : <p className="text-sm text-slate-500">无渠道数据</p>}</div></section>

      <section><h3 className="font-semibold text-slate-950">净业绩计算过程</h3><div className="mt-3 rounded-xl bg-slate-50 p-4 text-sm leading-7 text-slate-700"><p className="m-0">净业绩 = 入金 {money(evidence.financialFormula.rechargeCents)} − 出金 {money(evidence.financialFormula.withdrawalCents)} = <strong>{money(evidence.financialFormula.netPerformanceCents)}</strong></p></div></section>

      <section><h3 className="font-semibold text-slate-950">趋势</h3><div className="analysis-detail-grid mt-3"><p className="rounded-lg bg-slate-50 p-3 text-sm">渠道校正效率变化<strong className="mt-1 block text-lg">{percent(evidence.trend.adjustedEfficiencyChange)}</strong></p><p className="rounded-lg bg-slate-50 p-3 text-sm">净业绩变化<strong className="mt-1 block text-lg">{money(evidence.trend.netPerformanceChangeCents)}</strong></p></div></section>
