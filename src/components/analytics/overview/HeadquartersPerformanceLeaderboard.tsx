...
type EliteRow = { key: string; name: string; subtitle: string; orders: number; profitCents: number | null };
...
function EliteCard({ row, rank, kind }: { row?: EliteRow; rank: 1 | 2 | 3; kind: "orders" | "performance" }) {
  ...
  const value = row ? kind === "orders" ? `${row.orders} 单` : money(row.profitCents) : null;
...
export function HeadquartersPerformanceLeaderboard({ performanceRows, updatedAtLabel }: { performanceRows: NonNullable<ManagementOverview["groupComparison"]>; filters?: Partial<AnalysisFilters>; allowDrilldown?: boolean; updatedAtLabel?: string }) {
  ...
  const rows: EliteRow[] = ranking.groups.map((row) => ({ key: row.groupId, name: row.groupName, subtitle: [row.departmentName, row.countryCode || "未设国家"].join(" · "), orders: row.orders, profitCents: row.profitCents }));
  const byOrders = [...rows].sort((left, right) => right.orders - left.orders || left.name.localeCompare(right.name, "zh-CN"));
  const byPerformance = [...rows].sort((left, right) => (right.profitCents ?? Number.NEGATIVE_INFINITY) - (left.profitCents ?? Number.NEGATIVE_INFINITY) || right.orders - left.orders || left.name.localeCompare(right.name, "zh-CN"));
  return <div className="leaderboard-workspace space-y-3">
    ...
    <EliteSection title="小组业绩榜 TOP 3" description="按小组计入业绩排名：入金－出金－数据成本－底料返点。" rows={byPerformance} kind="performance" />
  </div>;
}
