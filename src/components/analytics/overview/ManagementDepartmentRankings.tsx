...
function RankingTable({ title, rows }: { title: string; rows: DepartmentRow[] }) {
  return <section className="panel overflow-hidden" aria-label={title}>
    ...
    <div className="data-table-wrap"><table className="data-table leaderboard-table"><thead><tr><th>排名</th><th>部门</th><th>小组数</th><th>开单</th><th>入金</th><th>出金</th><th>净业绩</th></tr></thead><tbody>
      {rows.map((row, index) => <tr key={row.departmentId}><td><span className="leaderboard-list-rank" data-top={index < 3}>{index + 1}</span></td><td><strong>{row.departmentName}</strong></td><td>{row.groupCount}</td><td className="font-semibold">{row.orders}</td><td>{money(row.rechargeCents)}</td><td>{money(row.withdrawalCents)}</td><td className={row.netPerformanceCents < 0 ? "font-semibold text-red-700" : "font-semibold"}>{money(row.netPerformanceCents)}</td></tr>)}
...
export function ManagementDepartmentRankings({ performanceRows }: { performanceRows: NonNullable<ManagementOverview["groupComparison"]> }) {
  ...
  const orders = [...departments].sort((left, right) => right.orders - left.orders || right.netPerformanceCents - left.netPerformanceCents || left.departmentName.localeCompare(right.departmentName, "zh-CN"));
  const performance = [...departments].sort((left, right) => right.netPerformanceCents - left.netPerformanceCents || right.orders - left.orders || left.departmentName.localeCompare(right.departmentName, "zh-CN"));
