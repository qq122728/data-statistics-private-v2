      ...
      <div className="member-phone-paste">
            ...
            <div className="member-import-customer-list" aria-label="客户资料录入表">
              ...
              {importRows.map((row, index) => <section className="member-import-customer-row" key={row.id} aria-label={`第 ${index + 1} 位客户资料`}>
                <label><span className="member-import-cell-label">客户编号 *</span><input aria-label={`${row.id} 客户编号`} value={row.phone} onChange={(event) => updateImportRow(row.id, { phone: event.target.value })} placeholder="输入任意长度，自动保留末6位" inputMode="numeric" /></label>
