      ...
      messages: [
        {
          ...
          content: [
            ...
            "业务词映射：接粉/添加→dispatchCount，撞粉→duplicateCount，低金额/小金额→lowAmountCount，无号码/无WS→noWsCount，人工无效→manualInvalidCount，回复→replyCount，进群→joinCount，正常退群→normalLeaveCount，异常退群→abnormalLeaveCount，推专家→expertIntroCount，注册→registrationCount，开单→orderCount，首充→InitialDeposit，续充/再次入金→Recharge，出金→withdrawalCents。",
            ...
            `当前统计日是 ${options.today ?? "未知"}。老客户或其他日期的粉今天发生进群、开单或续充，输出 legacy_event。`,
