...
async function activeCounts(tx: Prisma.TransactionClient, userId: string, groupId: string): Promise<Counts> {
  ...
  const [reception, operator, expert] = await Promise.all([
    tx.leadCustomer.count({ where: { AND: [customerCurrentGroupWhere(groupId), activeTracking], ownerId: userId, joinedOn: null, receptionArchivedAt: null } }),
    tx.leadCustomer.count({ where: { AND: [customerCurrentGroupWhere(groupId), activeTracking], groupOperatorOwnerId: userId, expertIntroducedOn: null, leftOn: null } }),
    tx.leadCustomer.count({ where: { AND: [customerCurrentGroupWhere(groupId), activeTracking], expertOwnerId: userId, customerOrder: null, OR: [{ expertWorkflowStage: null }, { expertWorkflowStage: { notIn: ["STALLED", "DECLINED_DEPOSIT"] } }] } }),
  ]);
...
async function disableFormerLead(tx: Prisma.TransactionClient, plan: Awaited<ReturnType<typeof tx.groupLeadChangePlan.findUniqueOrThrow>>, actorId: string) {
  ...
  await Promise.all([
    ...
    tx.leadCustomer.updateMany({ where: { AND: [customerCurrentGroupWhere(plan.groupId), activeTracking], ownerId: plan.formerLeadId, joinedOn: null, receptionArchivedAt: null }, data: { ownerId: successorId } }),
    tx.leadCustomer.updateMany({ where: { AND: [customerCurrentGroupWhere(plan.groupId), activeTracking], groupOperatorOwnerId: plan.formerLeadId, expertIntroducedOn: null, leftOn: null }, data: { groupOperatorOwnerId: successorId } }),
    tx.leadCustomer.updateMany({ where: { AND: [customerCurrentGroupWhere(plan.groupId), activeTracking], expertOwnerId: plan.formerLeadId, customerOrder: null, OR: [{ expertWorkflowStage: null }, { expertWorkflowStage: { notIn: ["STALLED", "DECLINED_DEPOSIT"] } }] }, data: { expertOwnerId: successorId } }),
  ]);
}
