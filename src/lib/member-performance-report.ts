...
const money = (value: number | null) => formatUsdOr(value, "待设成本");
...
function receptionRow(row: ReceptionRankingRow): MemberPerformanceReportRow {
  ...
  return {
    ...
    netPerformance: money(row.profitCents), dataCost: money(row.costCents),
  };
}
...
function operatorRow(row: GroupOperatorRankingRow): MemberPerformanceReportRow {
  return {
    ...
    firstDeposit: money(row.firstDepositCents), deposit: money(row.depositCents), withdrawal: money(row.withdrawalCents), netPerformance: money(row.netCents), dataCost: "—",
  };
}
...
const columns: Array<[keyof MemberPerformanceReportRow, string]> = [
  ...
  ["introduced", "推专家"], ["contacted", "专家已联系"], ["registered", "注册"], ["registrationRate", "注册率"],
    ["orders", "开单"], ["orderRate", "开单率"], ["firstDeposit", "首充（美元）"], ["deposit", "入金（美元）"], ["withdrawal", "出金（美元）"], ["netPerformance", "净业绩（美元）"], ["dataCost", "数据成本（美元）"],
];
