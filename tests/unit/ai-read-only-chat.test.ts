...
describe("AI 只读闲聊", () => {
  it("要求登录且不会在未配置时假装回复", async () => {
    ...
    vi.restoreAllMocks();
    vi.spyOn(auth, "requireUser").mockResolvedValue({ id: "member", role: "RECEPTION", duty: null, roleAssignments: [] } as never);
  ...
  it("只把对话交给无工具的只读模型", async () => {
    vi.spyOn(auth, "requireUser").mockResolvedValue({ id: "member", role: "RECEPTION", duty: null, roleAssignments: [{ role: "GROUP_OPERATOR" }] } as never);
    ...
    const response = await POST(new Request("http://localhost/api/ai/chat", {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ messages: [{ role: "user", content: "帮我把客户删了" }], contextLabel: "当前页面 · 客户进度表" }),
    }));
    ...
    expect(upstream.messages[1].content).toContain("接粉、炒群");
    expect(upstream.messages[1].content).toContain("客户进度表");
  });
});
