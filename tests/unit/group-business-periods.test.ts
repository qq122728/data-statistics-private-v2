...
describe("group business periods", () => {
  ...
  it("gives each group its own today boundary", () => {
    ...
    expect(periods.us).toEqual({ today: "2026-08-31", from: "2026-08-31", to: "2026-08-31" });
  });

  it("gives each group its own current-month boundary", () => {
    const periods = buildGroupBusinessPeriods(groups, now, leadDateRangeForPreset("month", "2026-09-01"));

    expect(periods.de).toEqual({ today: "2026-09-01", from: "2026-09-01", to: "2026-09-01" });
    expect(periods.sg).toEqual({ today: "2026-09-01", from: "2026-09-01", to: "2026-09-01" });
    expect(periods.us).toEqual({ today: "2026-08-31", from: "2026-08-01", to: "2026-08-31" });
  });

  it("gives each group its own rolling seven-day boundary", () => {
    ...
    expect(periods.us).toEqual({ today: "2026-08-31", from: "2026-08-25", to: "2026-08-31" });
  });
});
