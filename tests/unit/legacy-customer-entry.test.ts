...
afterEach(async () => {
  ...
  await db.dailyStatRevision.deleteMany({
    where: { entry: { groupId: { startsWith: prefix } } },
  });
...
describe.sequential("legacy customer entry", () => {
  it("does not let correction actions erase the imported historical baseline", () => {
    ...
    expect(
      buildBasicCustomerMutation(
        { action: "undoReply", reason: "误点" },
        lead,
        "2026-08-26",
      ),
    ).toMatchObject({
      status: 400,
      error: expect.stringContaining("历史底账"),
    });
    expect(
      buildBasicCustomerMutation(
        { action: "undoJoinGroup", reason: "误点" },
        lead,
        "2026-08-26",
      ),
    ).toMatchObject({
  ...
  it("opens the unified endpoint to frontline roles but validates the request body", async () => {
    ...
    await db.department.create({
      data: { id: departmentId, name: `${prefix}公司-${suffix}` },
    });
    await db.teamGroup.create({
      data: { id: groupId, name: `${prefix}小组-${suffix}`, departmentId },
    });
    const createUser = (
      ...
      db.user.create({
        data: {
          id: `${prefix}${role}-${suffix}`,
          username: `${prefix}${role}-${suffix}`,
          name,
          role,
          groupId,
    ...
    const [lead, reception, expert] = await Promise.all([
      createUser("LEAD", "组长"),
      createUser("RECEPTION", "接粉员"),
      createUser("EXPERT", "专家"),
    ]);
    vi.spyOn(auth, "requireUser").mockResolvedValue(lead);

    const response = await POST(
      new Request("http://localhost/api/legacy-customers", {
        method: "POST",
    ...
    await expect(
      db.leadCustomer.count({ where: { batch: { groupId } } }),
    ).resolves.toBe(0);
  });
  ...
  it("counts only post-cutover progress while never adding historical fan volume", async () => {
    ...
    const saved = await db.leadCustomer.findUniqueOrThrow({
      where: { phone: normalizeCustomerPhone(phone) },
    ...
    expect(saved.activities.map((activity) => activity.kind).sort()).toEqual([
      "JOINED_GROUP",
      "REPLIED",
    ]);
    ...
    await expect(
      ...
      operatorReceivedCount: 1,
      currentInGroupCount: 1,
    });
    await expect(
      db.dailyStatRevision.count({
        where: {
          entry: { groupId, businessDate: "2026-08-26", position: "RECEPTION" },
          joinCount: { gt: 0 },
        },
      }),
    ).resolves.toBe(0);
    await expect(
      db.sourceBatch.findFirstOrThrow({ where: { groupId } }),
    ).resolves.toMatchObject({ sourceDate: "2026-08-18" });
  ...
  it("records an old customer's order today without backfilling its earlier funnel", async () => {
    ...
    await db.channel.create({
      data: {
        ...
        groupId,
        name: "历史渠道",
        normalizedName: `history-order-${suffix}`,
      },
    });
    ...
    await expect(
      db.leadCustomer.findUniqueOrThrow({
        where: { phone: normalizeCustomerPhone(phone) },
        include: { batch: true, customerOrder: true },
      }),
    ).resolves.toMatchObject({
      batch: { sourceDate: "2026-08-10" },
      ...
      expertIntroducedOn: "2026-08-15",
      registeredOn: "2026-08-18",
      customerOrder: { openedOn: "2026-08-26" },
    });
    ...
    expect(facts.filter((fact) => fact.kind === "ORDER")).toMatchObject([
      { occurredOn: "2026-08-26", quantity: 1 },
    ]);
    expect(facts.filter((fact) => fact.kind === "RECHARGE")).toMatchObject([
      { occurredOn: "2026-08-26", amountCents: 100_000 },
    ]);
    await expect(
      db.dailyStatRevision.findFirstOrThrow({
        where: {
          entry: { groupId, businessDate: "2026-08-26", position: "EXPERT" },
        },
      }),
    ).resolves.toMatchObject({ orderCount: 1, cryptoInitialDepositCents: 0 });
  });
  ...
  it("records only today's recharge for a customer who opened before the system", async () => {
    ...
    await db.channel.create({
      data: {
        ...
        groupId,
        name: "历史渠道",
        normalizedName: `history-recharge-${suffix}`,
      },
    });
    ...
    const response = await POST(
      new Request("http://localhost/api/legacy-customers", {
        ...
        body: JSON.stringify({
          ...
          joinedOn: "2026-07-15",
          expertIntroducedOn: "2026-07-20",
          registeredOn: "2026-08-01",
          openedOn: "2026-08-10",
          baselineStage: "ORDERED",
          baselineOn: "2026-08-10",
          currentEvent: "RECHARGE",
    ...
    expect(facts.filter((fact) => fact.kind === "RECHARGE")).toMatchObject([
      { occurredOn: "2026-09-01", amountCents: 50_000 },
    ]);
    await expect(
      db.dailyStatEntry.count({
        where: { groupId, businessDate: "2026-09-01", position: "EXPERT" },
      }),
    ).resolves.toBe(0);
  });
  ...
  it("simulates Aug 30 old-fan join, Aug 29 old-fan order, and a prior order recharging on Sep 1", async () => {
    ...
    await db.channel.create({
      data: {
        ...
        groupId,
        name: "8月底历史渠道",
        normalizedName: `history-combined-${suffix}`,
      },
    });
    vi.spyOn(auth, "requireUser").mockResolvedValue(expert);
    ...
    const joined = await post({
      ...
      baselineStage: "REPLIED",
      baselineOn: "2026-08-30",
      currentEvent: "JOINED",
      occurredOn: "2026-09-01",
      deviceCode: "TEST-WA-01",
    });
    const ordered = await post({
      ...common,
      phone: phones.ordered,
      sourceDate: "2026-08-29",
      joinedOn: "2026-08-29",
      expertIntroducedOn: "2026-08-30",
      registeredOn: "2026-08-31",
      baselineStage: "REGISTERED",
      baselineOn: "2026-08-31",
      currentEvent: "ORDERED",
      occurredOn: "2026-09-01",
      initialDepositCents: 123_400,
      initialDepositMethod: "CRYPTO",
    });
    const recharged = await post({
      ...common,
      phone: phones.recharged,
      sourceDate: "2026-07-15",
      joinedOn: "2026-07-20",
      expertIntroducedOn: "2026-08-01",
      registeredOn: "2026-08-15",
      openedOn: "2026-08-29",
      baselineStage: "ORDERED",
      baselineOn: "2026-08-29",
      currentEvent: "RECHARGE",
      occurredOn: "2026-09-01",
      amountCents: 50_000,
      initialDepositMethod: "BANK",
    });
    expect([joined.status, ordered.status, recharged.status]).toEqual([
      201, 201, 201,
    ]);

    const saved = await db.leadCustomer.findMany({
      where: {
        phone: { in: Object.values(phones).map(normalizeCustomerPhone) },
      },
      include: { batch: true, customerOrder: { include: { events: true } } },
      orderBy: { phone: "asc" },
    });
    expect(saved).toHaveLength(3);
    expect(saved.map((row) => row.batch.sourceDate).sort()).toEqual([
      "2026-07-15",
      "2026-08-29",
      "2026-08-30",
    ]);
    expect(
      saved.find((row) => row.phone === normalizeCustomerPhone(phones.joined)),
    ).toMatchObject({ joinedOn: "2026-09-01", isHistoricalRecord: true });
    expect(
      saved.find((row) => row.phone === normalizeCustomerPhone(phones.ordered))
        ?.customerOrder,
    ).toMatchObject({
      openedOn: "2026-09-01",
      initialDepositCents: 123_400,
    ...
    expect(
      saved.find(
        (row) => row.phone === normalizeCustomerPhone(phones.recharged),
      )?.customerOrder,
    ).toMatchObject({
      openedOn: "2026-08-29",
      initialDepositCents: 0,
    ...
    const operatorEntry = await db.dailyStatEntry.findFirstOrThrow({
      where: {
        groupId,
    ...
    const expertEntry = await db.dailyStatEntry.findFirstOrThrow({
      where: { groupId, businessDate: "2026-09-01", position: "EXPERT" },
      include: { currentRevision: true },
    });
    await expect(db.dailyStatEntry.count({
      where: { groupId, businessDate: "2026-09-01", position: "RECEPTION" },
    })).resolves.toBe(0);
    expect(operatorEntry.currentRevision).toMatchObject({
      operatorReceivedCount: 1,
      currentInGroupCount: 1,
    });
    expect(expertEntry.currentRevision).toMatchObject({
      orderCount: 1,
    ...
    await expect(
      db.dailyStatEntry.count({
        where: { groupId, businessDate: { in: ["2026-08-29", "2026-08-30"] } },
      }),
    ).resolves.toBe(0);
    ...
    expect(
      facts.filter((fact) =>
        ["NEW_FANS", "EFFECTIVE_FANS"].includes(fact.kind),
      ),
    ).toHaveLength(0);
    expect(
      facts.filter(
        (fact) =>
          fact.kind === "GROUP_JOIN" && fact.occurredOn === "2026-09-01",
      ),
    ).toHaveLength(1);
    expect(
      facts.filter(
        (fact) => fact.kind === "ORDER" && fact.occurredOn === "2026-09-01",
      ),
    ).toHaveLength(1);
    expect(
      facts.filter(
        (fact) => fact.kind === "RECHARGE" && fact.occurredOn === "2026-09-01",
      ),
    ).toHaveLength(2);
  });
  ...
  it("keeps cross-team duplicate lookup generic", async () => {
    ...
    const actor = await db.user.create({
      data: {
        id: `${prefix}actor-${suffix}`,
        username: `${prefix}actor-${suffix}`,
        name: "甲组接粉",
        role: "RECEPTION",
        groupId: ownGroup.id,
        passwordHash: hashPassword("Legacy@56790"),
    ...
    const otherOwner = await db.user.create({
      data: {
        id: `${prefix}other-owner-${suffix}`,
        username: `${prefix}other-owner-${suffix}`,
        name: "绝不能泄露的乙组姓名",
        role: "RECEPTION",
    ...
    const channel = await db.channel.create({
      data: {
        id: `${prefix}other-channel-${suffix}`,
        groupId: otherGroup.id,
        name: "绝不能泄露的渠道",
        normalizedName: `other-${suffix}`,
      },
    });
    ...
    await db.leadCustomer.create({
      data: {
        ...
        customerName: "绝不能泄露的客户姓名",
      },
    });
    vi.spyOn(auth, "requireUser").mockResolvedValue(actor);

    const response = await GET(
      new Request(`http://localhost/api/legacy-customers?phone=${phone}`),
    );
